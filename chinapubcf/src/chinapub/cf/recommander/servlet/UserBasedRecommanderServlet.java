...
public class UserBasedRecommanderServlet extends HttpServlet {
    ...
    private static final int DEFAULT_HOW_MANY = 20; 
	    ...
	    private Recommender userrecommander; 
    	  //private SlopeOneRecommander recommender;
    //private ItemBasedRecommander recommender;
	    ...
	    public void init(ServletConfig config) throws ServletException { 
	        ...
	        if(userrecommenderClassName == null) { 
			 ...
			 try { 
				 // ChinapubRecommanderSingleton.initializeIfNeeded(recommenderClassName); 
	    ...
	    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException { 
	        ...
	        try { 
	            // 涓烘寚瀹氱敤鎴疯绠楁帹鑽愮殑鍥句功
	            List<RecommendedItem> useritems = userrecommander.recommend(userID, howMany); 
	            // 鍔犺浇鍥句功鐨勭浉鍏充俊鎭紝RecommendBookList 鏄繚瀛樹簡涓�粍鎺ㄨ崘鍥句功鐨勭浉鍏充俊鎭拰
	            // 寮曟搸璁＄畻寰楀埌鐨勪粬浠殑 ranking 
	            RecommandBookList bookList = new RecommandBookList(useritems); 
	            if ("text".equals(format)) { 
	                writePlainText(response, userID, debug, useritems, bookList); 
	            } else if ("xml".equals(format)) {
					writeXML(response, useritems);
		...
		private void writeDebugRecommendations(long userID,Iterable<RecommendedItem> items, PrintWriter writer)throws TasteException {
			DataModel udataModel = userrecommander.getDataModel();
			writer.print("User:");
			writer.println(userID);
			writer.print("UserRecommander: ");
			writer.println(userrecommander);
			writer.println();
			writer.print("Top ");
			writer.print(NUM_TOP_PREFERENCES);
			writer.println(" Preferences:");
			PreferenceArray urawPrefs = udataModel.getPreferencesFromUser(userID);
		//未完待补
			int length =20;// rawPrefs.length();
			PreferenceArray sortedPrefs = urawPrefs.clone();
		...
		public String toString() {
			return "RecommenderServlet[userrecommander:" + userrecommander +"]";
		}
