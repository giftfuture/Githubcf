...
public class SlopeOneRecommanderServlet extends HttpServlet {
    ...
    private static final int DEFAULT_HOW_MANY = 20; 
	    ...
	    public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException { 
	        ...
	        try { 
	            // 涓烘寚瀹氱敤鎴疯绠楁帹鑽愮殑鍥句功
	            List<RecommendedItem> slopeitems = sloperecommander.recommend(userID, howMany); 
	            // 鍔犺浇鍥句功鐨勭浉鍏充俊鎭紝RecommendBookList 鏄繚瀛樹簡涓�粍鎺ㄨ崘鍥句功鐨勭浉鍏充俊鎭拰
	            // 寮曟搸璁＄畻寰楀埌鐨勪粬浠殑 ranking 
	            RecommandBookList bookList = new RecommandBookList(slopeitems); 
	            if ("text".equals(format)) { 
	                writePlainText(response, userID, debug, slopeitems, bookList); 
	            } else if ("xml".equals(format)) {
					writeXML(response, slopeitems);
		...
		private void writeDebugRecommendations(long userID,Iterable<RecommendedItem> items, PrintWriter writer)throws TasteException {
			DataModel sdataModel = sloperecommander.getDataModel();
			writer.print("User:");
			writer.println(userID);
			writer.print("SlopeOneRecommander: ");
			writer.println(sloperecommander);
			writer.println();
			writer.print("Top ");
			writer.print(NUM_TOP_PREFERENCES);
			writer.println(" Preferences:");
			PreferenceArray srawPrefs = sdataModel.getPreferencesFromUser(userID);
		//未完待补
			int length =20;// rawPrefs.length();
			PreferenceArray sortedPrefs = srawPrefs.clone();
		...
		public String toString() {
			return "RecommenderServlet[slopeonerecommander:"+sloperecommander+"]";
		}
